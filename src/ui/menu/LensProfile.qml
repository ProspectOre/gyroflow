...
MenuItem {
    ...
    function applyDetectedCamera(): void {
        ...
        cameraLens.currentIndex = 0;
        compatibleCameras = [];
        const resolved = controller.camera_database_resolve_camera(detectedCamera.brand, detectedCamera.model);
        const brand = resolved.brand || detectedCamera.brand;
        const model = resolved.model || detectedCamera.model;
